...
package imagingbook.parboiled.scala.parserunners

import imagingbook.parboiled.common.{Predicate, Sink, Tuple2}
import imagingbook.parboiled.support.MatcherPath
import imagingbook.parboiled.{Context, parserunners}
...
class TracingParseRunner[V](val inner: parserunners.TracingParseRunner[V]) extends ParseRunner[V] {
  ...
  def filter(filter: TracingPredicate) = {
    inner.withFilter(new Predicate[Tuple2[Context[Any], Boolean]] {
      def apply(t: Tuple2[Context[Any], Boolean]) = filter((t.a, t.b))
    })
    this
  }
