...
import org.parboiled.scala.{Input, ParsingResult}
...
object RecoveringParseRunner {
  ...
  def apply(rule: Rule0, timeout: Long) = {
    new RecoveringParseRunner[Nothing](new parserunners.RecoveringParseRunner[Nothing](rule, timeout))
  }
  ...
  def apply[V](rule: Rule1[V], timeout: Long = Long.MaxValue) = {
    new RecoveringParseRunner[V](new parserunners.RecoveringParseRunner[V](rule, timeout))
  }
}

class RecoveringParseRunner[V](val inner: parserunners.RecoveringParseRunner[V]) extends ParseRunner[V] {
