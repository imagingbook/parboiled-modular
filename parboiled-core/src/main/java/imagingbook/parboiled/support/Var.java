...
package imagingbook.parboiled.support;

import imagingbook.parboiled.common.Factory;
import imagingbook.parboiled.common.Reference;
 ...
 * {@literal Var<T>} objects wrap an internal value of an arbitrary (reference) type, can have an initial value,
 * allow read/write access to their values and can be passed around as parameters to nested rule methods.
 * Each rule invocation (i.e. rule matching attempt) receives its own {@literal Var<T>} scope (which is automatically
 * initialized with the initial value), so actions in recursive rules work just like expected.</p>
 * <p>{@literal Var<T>} objects generally behave just like local variables with one exception:<br/>
 ...
 * value (since values in {@literal Var<T>} objects are passed by reference)</p>
 *
