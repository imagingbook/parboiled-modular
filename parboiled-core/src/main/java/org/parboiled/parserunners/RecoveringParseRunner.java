    ...
    private boolean fixError(int fixIndex) {
        ...
        if (nextErrorAfterBestSingleCharFix > fixIndex) {
            ...
            if (nextErrorAfterBestSingleCharFix == nextErrorAfterDeletion) {
                buffer.insertChar(fixIndex, Chars.DEL_ERROR);
                ...
                buffer.insertChar(fixIndex, Chars.INS_ERROR);
                ...
                buffer.insertChar(fixIndex + 1, Chars.INS_ERROR);
                buffer.insertChar(fixIndex, Chars.DEL_ERROR);
            ...
            if (buffer.charAt(fixIndex) == Chars.EOI) {
                buffer.insertChar(fixIndex, Chars.RESYNC_EOI);
                currentError.shiftIndexDeltaBy(1);
                return true;
            }
            buffer.insertChar(fixIndex, Chars.RESYNC);
            currentError.shiftIndexDeltaBy(1);
    ...
    private class Handler implements MatchHandler {
        ...
        private boolean resynchronize(MatcherContext context) {
            ...
            switch (context.getCurrentChar()) {
                case Chars.RESYNC:
                    ...
                    buffer.replaceInsertedChar(currentError.getStartIndex() - 1, Chars.RESYNC_START);
                    buffer.insertChar(endIndex, Chars.RESYNC_END);
                ...
                case Chars.RESYNC_START:
                    ...
                    while (context.getCurrentChar() != Chars.RESYNC_END) {
                        context.advanceIndex(1); // skip all characters up to the RESYNC_END
                        checkState(context.getCurrentChar() != Chars.EOI); // we MUST find a RESYNC_END before EOI
                    }
                    context.advanceIndex(1); // gobble RESYNC_END marker
                    break;
        ...
        private int gobbleIllegalCharacters(MatcherContext context, List<Matcher> followMatchers) {
            ...
            while (true) {
                ...
                if (currentChar == Chars.EOI) break;
